...
- **emitters**: Record the generating castiron version in a header
  ([`94078ae`](https://github.com/kmbhm1/castiron/commit/94078ae41a1db112e763af076e3be976cd149cbd))

Every emitted module now opens with two ASCII comment lines and one blank line:

# Generated by castiron 0.5.0 # Do not edit by hand. Regenerate with `castiron gen` --
  https://github.com/kmbhm1/castiron

Why now, and why alone. `castiron check` (CI-021b) re-emits in memory and diffs against the
  committed output, so when castiron's own EMITTER improves the bytes move even though the user's
  schema did not -- every enum fix in the 0.1.x-0.2.0 series did exactly that. A byte-diff `check`
  would go red on a castiron upgrade with a perfectly correct schema, which is the single failure
  mode most likely to make people disable it. The recorded version is what lets `check` say
  "generated by 0.4.0, running 0.5.0 -- run gen to adopt" instead of just failing.

Adding a header moves every byte-committed artifact -- one unavoidable "everyone runs gen once"
  event. Shipping it before `check` exists spends that while nothing is watching, so `check` later
  debuts green. The reverse order spends it as a red CI build on every adopter.

The grammar is a permanent contract: the version is the last whitespace-delimited token of line 1,
  `^# Generated by castiron (\S+)$`, parsed by `parse_header_version` from line 1 only. Line 2 is
  prose and is never read, so any future release may reword it without orphaning files already
  written. `None` is a first-class result for a hand-written or pre-header file.

Byte-stability (Hard Rule #9) still holds. The header is emitted bytes: no timestamp, no hostname,
  no path, no source URL (that would write ?apikey= into a committed file) and no config summary.
  The castiron version is the only varying input and it is injected, never read ambiently at render
  time.

A comment block rather than a module docstring, for two reasons: a docstring becomes the user's
  __doc__, and CPython 3.13 dedents docstrings at compile time -- the defect that shipped CI red on
  one leg and widened `make validate` to the whole matrix. It precedes `from __future__ import
  annotations`, which is legal because __future__ must be the first STATEMENT and a comment is not
  one.

`tool_version` is a keyword-only constructor parameter, deliberately NOT an EmitterConfig field: the
  corpus derives its sweep from dataclasses.fields(EmitterConfig) and assigns a bool to every axis,
  so a field would widen it 128 -> 256 points and build EmitterConfig(tool_version=True).
  `comment_prefix` lets a future TS/Zod emitter render `//` through the same function instead of
  growing a second mechanism.

Every committed golden pins GOLDEN_TOOL_VERSION rather than the live version. semantic-release
  rewrites castiron.__version__ inside the release commit, so a live version in a golden would turn
  main red on every release, in a commit no developer can regenerate. The two real-CLI assertions
  still cover every byte: line 1 against castiron.__version__, lines 2+ against the golden.

Golden delta, predicted before regenerating and matched exactly: 6 module goldens +3 lines / +130
  chars, all 512 manifest rows re-hashed with lines +3 and chars +130,
  classes/fields/imports/compiles unchanged on every row, and all 5 ir.json goldens byte-unchanged.
  Strip the header and every golden is byte-identical to origin/main.

Docs: three published sentences promised "no generated-on banner" and "the same schema and the same
  options produce the same bytes". A version header is a generated-on banner, so all three are
  amended here rather than in a follow-up -- code and docs must not disagree at any commit -- and
  generated-code.md gains a section documenting the format and the parse contract.
