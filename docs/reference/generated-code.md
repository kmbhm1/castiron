...
version could change the bytes, no timestamp, and no source URL in the file — the same schema,
the same options **and the same castiron version** produce the same bytes, every time. The one
thing the file records about how it was made is the castiron version, on its first line; see
[The provenance header](#the-provenance-header) below. That is what makes generated models safe
to commit and to diff, and it is the foundation the planned `castiron check` drift-guard will
stand on.

## The provenance header

Every generated module opens with exactly two comment lines and one blank line:

```python
# Generated by castiron 0.5.0
# Do not edit by hand. Regenerate with `castiron gen` -- https://github.com/kmbhm1/castiron

from __future__ import annotations
```

It is a **comment block, not a module docstring**, so it never becomes your module's `__doc__`
and it never displaces `from __future__ import annotations` — a `__future__` import must be the
first *statement*, and a comment is not a statement.

**What is in it, and what deliberately is not.** The castiron version, and nothing else. No
timestamp: the file would then change on every run and stop being diffable. No source URL: a
`--from https://project.supabase.co/?apikey=…` would write a credential into a file you commit.
No configuration summary: every flag you flipped would show up as a header diff. The version is
the only varying input, which is what keeps the output byte-stable.

**Line 1 is a stable, machine-readable contract.** The version is the last whitespace-delimited
token of the first line, with no trailing punctuation:

```
# Generated by castiron <version>
```

Parse that and nothing else. Line 2 is prose and may be reworded in any release. A file
generated by one castiron will always be readable by a later one — which is the whole point of
recording the version: when `castiron check` lands it can tell you *"generated by castiron
0.5.0, running 0.6.0 — run `gen` to adopt"* instead of merely failing, so a castiron upgrade
does not look like a schema change.
