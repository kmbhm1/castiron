...
to commit and to diff, and it is the foundation [`castiron check`](cli.md#castiron-check) stands
on.
...
recording the version: `castiron check` tells you *"generated by castiron 0.5.0; you are running
0.6.0"* instead of merely failing, so a castiron upgrade does not look like a schema change.
