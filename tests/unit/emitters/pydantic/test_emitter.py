...
from castiron.emitters.base import parse_header_version, render_header
...
def _emit(schema: Schema, config: EmitterConfig | None = None) -> str:
    """Emit ``schema`` with the header version pinned to the corpus sentinel.

    ⚠ Pinned so that no assertion in this module -- and no committed golden it compares against --
    is a function of ``castiron.__version__``, which ``python-semantic-release`` rewrites in the
    release commit. ``TestProvenanceHeader`` covers the *unpinned* default separately.
    """
    return PydanticEmitter(config, tool_version=GOLDEN_TOOL_VERSION).emit(schema)[0].content


def _after_header(text: str) -> str:
    """The emitted module with its two-line provenance header and the blank line after it removed.

    Every structural assertion about the module body goes through this, so the header is asserted
    in exactly one place (``TestProvenanceHeader``) rather than smeared across the file.
    """
    header, separator, body = text.partition('\n\n')
    assert separator, 'the emitted module has no blank line after its header'
    assert parse_header_version(header) is not None, f'the module does not open with a header: {header!r}'
    return body
...
def _import_block(text: str) -> str:
    ...
    ``#`` -- so the split point is exact rather than heuristic. ⚠ The provenance header is
    stripped first: it is itself a ``#`` block, so searching the raw text would find *it* and
    report an empty import block.
    """
    lines = _after_header(text).splitlines()
...
class TestProvenanceHeader:
    """CI-021a: the two lines that let ``castiron check`` tell a castiron upgrade from a schema change."""

    def test_it_opens_with_a_two_line_header_then_one_blank_line(self, representative_schema: Schema) -> None:
        lines = _emit(representative_schema).splitlines()
        assert lines[0] == f'# Generated by castiron {GOLDEN_TOOL_VERSION}'
        assert lines[1] == '# Do not edit by hand. Regenerate with `castiron gen` -- https://github.com/kmbhm1/castiron'
        assert lines[2] == ''
        assert lines[3] == 'from __future__ import annotations'

    def test_the_header_is_exactly_what_render_header_renders(self, representative_schema: Schema) -> None:
        # One renderer, not two: the emitter must not grow its own copy of the grammar.
        assert _emit(representative_schema).startswith(f'{render_header(GOLDEN_TOOL_VERSION)}\n\n')

    def test_the_recorded_version_round_trips_back_out(self, representative_schema: Schema) -> None:
        assert parse_header_version(_emit(representative_schema)) == GOLDEN_TOOL_VERSION

    def test_the_default_version_is_the_installed_castiron(self, representative_schema: Schema) -> None:
        # 🔴 The one assertion that binds the header to reality. Every other test in this module
        # pins the sentinel, so without this the emitter could record anything at all and the
        # whole suite would stay green.
        emitted = PydanticEmitter(EmitterConfig()).emit(representative_schema)[0].content
        assert parse_header_version(emitted) == castiron.__version__

    def test_changing_only_the_version_changes_exactly_one_line(self, representative_schema: Schema) -> None:
        # 🔴 The direct proof of the premise the version-aware `check` diagnostic (CI-021b §5.5)
        # rests on: a castiron upgrade perturbs ONE line, so a drift report with more than one
        # changed line is genuinely the user's schema or a hand edit.
        first = PydanticEmitter(EmitterConfig(), tool_version='1.2.3').emit(representative_schema)[0].content
        second = PydanticEmitter(EmitterConfig(), tool_version='4.5.6').emit(representative_schema)[0].content
        changed = [
            line
            for line in difflib.unified_diff(first.splitlines(), second.splitlines(), n=0, lineterm='')
            if line.startswith(('+', '-')) and not line.startswith(('+++', '---'))
        ]
        assert changed == ['-# Generated by castiron 1.2.3', '+# Generated by castiron 4.5.6']

    def test_the_header_does_not_make_the_module_a_docstring_carrier(self, representative_schema: Schema) -> None:
        # ⚠ A comment, never a module docstring. CPython 3.13 dedents docstrings at compile time
        # -- the CI-082 one-leg-red trap -- and a docstring would also become the user's `__doc__`.
        assert ast.get_docstring(ast.parse(_emit(representative_schema))) is None

    def test_the_module_with_a_header_still_compiles(self, representative_schema: Schema) -> None:
        compile(_emit(representative_schema), '<generated>', 'exec')

    def test_the_header_carries_no_ambient_state(self, representative_schema: Schema) -> None:
        # Hard Rule #9: the version is the ONLY varying input. No timestamp, no path, no host --
        # so two emissions a moment apart, from two emitter instances, are byte-identical.
        assert _emit(representative_schema) == _emit(representative_schema)

    def test_it_is_pure_ascii(self, representative_schema: Schema) -> None:
        # castiron-authored bytes in someone else's repository add no encoding question of their
        # own (the body already carries the user's non-ASCII SQL comments). Hence `--`, not an em dash.
        header = _emit(representative_schema).partition('\n\n')[0]
...
class TestDeterminism:
    ...
    def test_exactly_one_blank_line_separates_the_imports_from_the_body(self, representative_schema: Schema) -> None:
        ...
        out = _after_header(_emit(representative_schema))
        block = _import_block(_emit(representative_schema))
        assert out.startswith(f'{block}\n\n#')
        assert not out.startswith(f'{block}\n\n\n')

    def test_future_annotations_is_the_first_statement(self, representative_schema: Schema) -> None:
        # ⚠ Reworded, not deleted, when CI-021a put a comment header above it. `__future__` must
        # be the first *statement*, and a comment is not a statement -- so the property that
        # actually matters is asserted against the AST, and the textual form is asserted against
        # the module with its header stripped.
        out = _emit(representative_schema)
        assert _after_header(out).startswith('from __future__ import annotations\n')
