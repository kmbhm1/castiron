...
#: Version spellings the header must survive: a release, a pre-release, and a local dev build. The
#: last one is the interesting case -- ``+`` and ``.`` inside the token are exactly what a naive
#: ``split('.')`` parse would mangle, and a ``uv pip install -e .`` user really does get one.
HEADER_VERSIONS = ('0.5.0', '1.0.0rc1', '0.4.0.dev12+g1234abc')


@pytest.mark.unit
class TestProvenanceHeader:
    """CI-021a: the render/parse pair ``castiron check`` (CI-021b) reads a committed file back with."""

    @pytest.mark.parametrize('version', HEADER_VERSIONS)
    def test_it_round_trips_every_version_spelling(self, version: str) -> None:
        assert parse_header_version(render_header(version)) == version

    @pytest.mark.parametrize('version', HEADER_VERSIONS)
    def test_it_round_trips_when_a_module_body_follows(self, version: str) -> None:
        module = f'{render_header(version)}\n\nfrom __future__ import annotations\n'
        assert parse_header_version(module) == version

    def test_it_renders_two_lines_with_no_trailing_newline(self) -> None:
        # The same convention as `render_import_block`: the caller owns the separator.
        rendered = render_header('0.5.0')
        assert rendered.count('\n') == 1
        assert not rendered.endswith('\n')

    def test_the_version_is_the_last_token_of_line_one(self) -> None:
        # 🔴 The permanent contract. A file generated by 0.5.0 must still parse under a much later
        # castiron, and this shape -- last whitespace-delimited token, no trailing punctuation --
        # is what makes that parse total.
        line_one = render_header('0.5.0').splitlines()[0]
        assert line_one.split()[-1] == '0.5.0'
        assert line_one == '# Generated by castiron 0.5.0'

    def test_line_two_may_be_reworded_without_breaking_the_parse(self) -> None:
        # Forward tolerance: nothing reads line 2, so a future release may reword it freely.
        module = '# Generated by castiron 0.5.0\n# something entirely different\n\nx = 1\n'
        assert parse_header_version(module) == '0.5.0'

    def test_a_custom_comment_prefix_renders_and_parses(self) -> None:
        # Exists so a TS/Zod emitter reuses this block instead of growing a second mechanism.
        rendered = render_header('0.5.0', comment_prefix='//')
        assert rendered.startswith('// Generated by castiron 0.5.0\n// Do not edit by hand.')
        assert parse_header_version(rendered, comment_prefix='//') == '0.5.0'

    def test_a_python_header_does_not_parse_as_a_typescript_one(self) -> None:
        assert parse_header_version(render_header('0.5.0'), comment_prefix='//') is None

    @pytest.mark.parametrize(
        ('label', 'text'),
        [
            ('empty', ''),
            ('a headerless module', 'from __future__ import annotations\n\nx = 1\n'),
            ('a header on line 2', '\n# Generated by castiron 0.5.0\n'),
            ('lowercase generated', '# generated by castiron 0.5.0\n'),
            ('no version token', '# Generated by castiron\n'),
            ('trailing prose', '# Generated by castiron 0.5.0 on Tuesday\n'),
            ('no comment marker', 'Generated by castiron 0.5.0\n'),
            ('a stray carriage return', '# Generated by castiron 0.5.0\r\n'),
        ],
    )
    def test_it_returns_none_rather_than_raising(self, label: str, text: str) -> None:
        # ⚠ `None` is a first-class outcome, not an error: a hand-written module and a module
        # generated before this header existed both land here, and `check`'s correct response is
        # "no recorded provenance", never a traceback.
        assert parse_header_version(text) is None, label

    def test_the_rendered_line_ends_in_no_punctuation(self) -> None:
        # ⚠ The parse takes the token VERBATIM -- it cannot tell `0.5.0.` from a version, because
        # versions legitimately contain periods (`0.4.0.dev12`). So the no-trailing-punctuation
        # rule is enforced on the RENDERER, which is the only side that can honour it. A period
        # here would make every recorded version compare unequal to the real one, forever.
        for version in HEADER_VERSIONS:
            line_one = render_header(version).splitlines()[0]
            assert not line_one.endswith(('.', ',', ';', ':', ')')), line_one
            assert parse_header_version(line_one) == version

    def test_it_reads_only_the_first_line_of_a_large_module(self) -> None:
        module = render_header('0.5.0') + '\n\n' + '# Generated by castiron 9.9.9\n' * 100
        assert parse_header_version(module) == '0.5.0'
